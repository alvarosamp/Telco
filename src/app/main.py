...
import gradio as gr 
from src.serving.inference import predict  # Single source for inference logic#

#Intiizalie FastAPI applicatin
app = FastAPI(
    title = 'Telco Customer Churn Prediction Service',
    description = 'A production-ready serving application for Telco Customer Churn model using FastAPI and Gradio.',
    version = '1.0.0'
)

#Health check endpoint 
#Critical : Required AWS Application Load Balancer health checks
app.get("/")
def root():
    """Health check endpoint. For monitoring AWS Load Balancer health checks."""
    return {'status': 'ok'}

#Request data schema
#Pydantic model for automatic validation and documentation
